...
export class LikeService {
  ...
  async togglePostLike(
    ...
    const existingLike = await this.likeRepository.findOne({
      where: { postId, userId, targetType: LikeTargetType.POST },
    });
  ...
  async toggleCommentLike(
    ...
    const existingLike = await this.likeRepository.findOne({
      where: { commentId, userId, targetType: LikeTargetType.COMMENT },
    });
    ...
    if (existingLike) {
      if (reaction && existingLike.reaction !== reaction) {
        existingLike.reaction = reaction;
        await this.likeRepository.save(existingLike);
        return { liked: true, likesCount: comment.likesCount, reaction: existingLike.reaction };
      } else {
        await this.likeRepository.remove(existingLike);
        comment.likesCount = Math.max(0, comment.likesCount - 1);
        await this.commentRepository.save(comment);
        return { liked: false, likesCount: comment.likesCount, reaction: undefined };
      }
    } else {
      const like = this.likeRepository.create({
        commentId,
      ...
      await this.commentRepository.save(comment);
      return { liked: true, likesCount: comment.likesCount, reaction: like.reaction };
    }
  }
